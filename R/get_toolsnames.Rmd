...
grab_tools <- function(input_url, input_toolbox, input_toolset){
  ...
  df <- map_dfr(my_tables, function(my_table){
    ...
    if(mycolnames[1] %in% c("Tool","Tool name","Geoprocessing tools","tool")){
      ...
      outtable <- left_join(my_table,url_table, by = "tool")
      outtable
    } else if(mycolnames[1] %in% c("Toolsets","Toolset")){
      
      colnames(my_table)[1] <- "toolset"
      colnames(my_table)[2] <- "description"
      
      prepared <- left_join(my_table, url_table, by = c("toolset" = "tool")) %>%
        mutate(
          url_adjusted2 = toolset_url_adjuster(href),
          toolbox2 = input_toolbox
        ) %>% 
        select(url_adjusted2, toolbox2, toolset2 = toolset)
      
      prepared %>%
        pmap_dfr(function(url_adjusted2,toolbox2, toolset2){
          grab_tools(url_adjusted2, toolbox2, toolset2)
        })
    }
  })
...
tools <- toolsets %>%
  ...
  # filter(row > 120) %>%
  # filter(
  #   toolset != "Overlay Events", 
  #   toolbox != "Crime Analysis and Safety toolbox", 
  #   toolbox != "Data Interoperability toolbox",
  #   toolbox != "Editing toolbox",
  #   toolbox != "Geocoding toolbox",
  #   !(toolset == "Raster" & toolbox == "Data Management toolbox") 
  #   ) %>% 
