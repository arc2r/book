...
```{r}
# Function to get the last element of a string separated with sep
getlast <- function(inp, sep = "/"){
  st <- str_split(inp,sep)
...
toolboxes <- toolboxes %>%
  mutate(
    ...
    url_adjusted = case_when(
      # the pattern is different for one toolbox
...
toolbox_desc <- toolboxes %>%
  pmap_dfr(function(name, url, url_adjusted,last){
    Sys.sleep(0.5)
    desc <- tryCatch(
...
```{r}
toolsets <- toolboxes %>%
  pmap_dfr(function(name, url, url_adjusted, last, desc){
    Sys.sleep(0.5)
    mytable <- tryCatch(
      grab_toolsets(url_adjusted),
      error = function(e){tibble(.rows = 1)}
    )
    mytable <- mytable %>%
      mutate(toolbox = name)
