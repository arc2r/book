...
Now that `bezirke` has an assigned `CRS` (see \@ref{define-projection}), we can transform it into a new coordinate system. To emphasize this, will will visualize the dataset using `base::plot()` with the option `axes = TRUE`, to visualize the current coordinate system.  
