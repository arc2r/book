...
Dissolve operation with `sf` package does not save the data frame (other than the geometry column) by default without specifying how to aggregate the data. So, based on the geometry column of the given dataset, we compute below the area of each and every of the depicted polygons. To do so we use the `st_area` function.

```{r dissolve_02}
# Computing the area of the depicted polygons based on the geometry column
eire$area <- st_area(eire)
eire
```
Now we have data that’s meaningful to group. To do so we can just use `summarise()` to group the data and perform the dissolve:
...
# visualising the dissolved dataset
ggplot(eire_dissolved) + geom_sf()
```
